		...
		try {
			...
			while((configLine=fileReader.readLine())!= null) {
				...
				if(configParams.length == 4) {
					...
					String centraladdress = configParams[0];
					int centralport = Integer.parseInt(configParams[1]);
					ServerState serverState = ServerState.getInstance();
					serverState.setCentraladdress(centraladdress);
					serverState.setCentralport(centralport);;
				}
			}
			fileReader.close();	

			//			config = new Config(serverName,serverAddress,clientsPort,coordinationPort);
			//			ServerState serverState = ServerState.getInstance();
			//			serverState.setConfig(config);
			//			serverState.getLocalChatroomInfoMap().put("MainHall-"+serverName,new LocalChatroomInfo("MainHall-"+serverName,"") );
